...
Stage 1 filters are cheap (Alpaca-based): price, volume, RSI, SMA200, HV percentile.
Stage 1b: earnings proximity filter (one Finnhub call per symbol, but cheaper than
    full fundamental data and critical for pre-filtering before options chain calls).
...
def compute_hv_percentile(
    bars_df: pd.DataFrame,
    hv_window: int = 30,
    lookback: int = 252,
) -> float | None:
    """Compute HV percentile rank: current 30-day HV vs trailing 252-day distribution.

    Calculates a rolling series of 30-day historical volatility values over the
    lookback period, then ranks the most recent value as a percentile (0–100).

    A percentile of 80 means the current HV is higher than 80% of the HV values
    observed over the past year — indicating elevated implied premium opportunity.

    Args:
        bars_df: DataFrame with a 'close' column of daily close prices.
        hv_window: Window for each HV calculation (default 30 trading days).
        lookback: Number of trading days of history to build the distribution
            (default 252 ≈ 1 year).

    Returns:
        HV percentile as a float in [0, 100], or None if insufficient data
        (needs at least lookback + 1 close prices).
    """
    if len(bars_df) < lookback + 1:
        return None

    close = bars_df["close"].values
    log_returns = np.log(close[1:] / close[:-1])

    # Use only the last `lookback` returns
    returns_window = log_returns[-lookback:]

    # Compute rolling HV for each position in the window
    hv_series: list[float] = []
    for end_idx in range(hv_window, len(returns_window) + 1):
        window_returns = returns_window[end_idx - hv_window : end_idx]
        daily_std = np.std(window_returns, ddof=1)
        annualized_hv = daily_std * np.sqrt(252)
        hv_series.append(float(annualized_hv))

    if not hv_series:
        return None

    current_hv = hv_series[-1]
    rank = sum(1 for hv in hv_series if hv <= current_hv)
    percentile = (rank / len(hv_series)) * 100.0
...
def filter_hv_percentile(stock: ScreenedStock, config: ScreenerConfig) -> FilterResult:
    """Check if HV percentile meets minimum threshold.

    Higher HV percentile means current volatility is elevated relative to
    the past year — indicating better premium-selling opportunities.

    Args:
        stock: ScreenedStock with hv_percentile field populated.
        config: ScreenerConfig with technicals.hv_percentile_min.

    Returns:
        FilterResult with pass/fail and reason.
    """
    min_pct = config.technicals.hv_percentile_min

    if stock.hv_percentile is None:
        return FilterResult(
            filter_name="hv_percentile",
            passed=True,
            actual_value=None,
            threshold=min_pct,
            reason="No data — passing with neutral score",
    ...
    if stock.hv_percentile < min_pct:
        return FilterResult(
            filter_name="hv_percentile",
            passed=False,
            actual_value=stock.hv_percentile,
            threshold=min_pct,
            reason=f"HV percentile {stock.hv_percentile:.1f} below minimum {min_pct:.1f}",
        )
...
def filter_earnings_proximity(
    ...
    """Exclude stocks with upcoming earnings within the configured threshold.

    Selling options into earnings carries outsized binary risk. This filter
    removes stocks whose next earnings date is within N days.

    Args:
        stock: ScreenedStock with days_to_earnings field populated.
        config: ScreenerConfig with earnings.earnings_exclusion_days.

    Returns:
        FilterResult with pass/fail and reason.
    """
    exclusion_days = config.earnings.earnings_exclusion_days

    if stock.days_to_earnings is None:
        # No earnings data — pass with neutral assumption
        return FilterResult(
            filter_name="earnings_proximity",
            passed=True,
            actual_value=None,
            threshold=float(exclusion_days),
            reason="No earnings data — passing with neutral score",
        )

    if stock.days_to_earnings <= exclusion_days:
        return FilterResult(
            filter_name="earnings_proximity",
            passed=False,
            actual_value=float(stock.days_to_earnings),
            threshold=float(exclusion_days),
            reason=(
                f"Earnings in {stock.days_to_earnings} days "
                f"(within {exclusion_days}-day exclusion window)"
            ),
        )
...
def run_stage_1_filters(stock: ScreenedStock, config: ScreenerConfig) -> bool:
    """Run all 5 Stage 1 (cheap) filters and record results.

    Runs all filters regardless of individual outcomes. Appends each
    FilterResult to stock.filter_results. Includes HV percentile filter.
    ...
    Returns:
        True only if all 5 filters passed.
    """
    results = [
        ...
        filter_hv_percentile(stock, config),
    ]
