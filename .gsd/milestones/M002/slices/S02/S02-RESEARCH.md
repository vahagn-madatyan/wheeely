...
The biggest risk is the **dependency on S01**, which is supposed to deliver `ScreenedStock.perf_1m`, `compute_monthly_performance()`, and `run_pipeline(top_n=N)`. The roadmap marks S01 as complete, but STATE.md shows it still in "planning" and none of its deliverables exist in code. The S01 summary is a doctor-created placeholder. **S02 cannot be executed until S01 is truly complete.**
...
1. **CLI flag** — Add `--top-n` Typer option to `scripts/run_screener.py`, pass through to `run_pipeline(top_n=N)`. Follow the exact same `Annotated[type, typer.Option()]` pattern used by `--update-symbols`, `--verbose`, `--preset`, and `--config`. Test with `CliRunner` mocking `run_pipeline`.

2. **Display column** — Add "Perf 1M" column to `render_results_table()` in `screener/display.py`. Add a `fmt_signed_pct()` formatter (or inline format) since TOPN-05 requires sign prefixes (`+3.1%`, `-5.2%`) and the existing `fmt_pct()` omits the `+` for positive values. Test with `Console(file=StringIO())` capture pattern.

## Don't Hand-Roll

| Problem | Existing Solution | Why Use It |
|---------|------------------|------------|
| CLI option parsing | Typer `typer.Option()` | Already used for 4 options in `run_screener.py`; provides help text, type validation, defaults |
| Table column rendering | Rich `Table.add_column()` + `Table.add_row()` | Already used for 12 columns in `render_results_table()` |
| Percentage formatting | `fmt_pct()` in `screener/display.py` | Handles None→"N/A"; need signed variant for perf column |
...
- `tests/test_cli_screener.py` — 4 CLI tests using `CliRunner` with extensive `@patch` decorators. The `test_verbose_shows_filter_breakdown` test is the closest pattern for testing a new flag (patches pipeline + display, invokes with flag, asserts a function was called).
- `tests/test_display.py` — `_make_stock()` helper, `_all_pass_filters()`, `_capture_console()` — reusable test fixtures. `test_table_has_column_headers` pattern: render table, assert column header strings exist in output.
- `tests/test_options_chain.py:test_yield_column_in_results_table()` — Most recent column addition test. Creates stock with field set, renders table, asserts column header + formatted value in output. Follow this pattern exactly.
...
- `top_n` must be a positive integer when provided — Typer can enforce via `min=1` or manual validation.
...
- **Not validating top_n >= 1** — `--top-n 0` or `--top-n -5` would silently produce empty results. Use Typer's `min=1` constraint or explicit validation.
...
- **Column width overflow** — 13 columns may get tight at console width < 120. Test with realistic console width (120+). The existing 12-column layout works at 120.
...
- **`perf_1m` field shape uncertainty** — S01 context says percentage (e.g. `-5.2` for 5.2% decline), but the actual implementation hasn't been written. If S01 stores it differently (e.g. as decimal `0.052`), the display formatter needs adjustment.

## Skills Discovered

| Technology | Skill | Status |
|------------|-------|--------|
| Typer (CLI) | — | none found (stdlib-level, no skill needed) |
| Rich (tables) | — | none found (stdlib-level, no skill needed) |
| pytest | — | none found (well-known, no skill needed) |

No external technologies new to this slice. All work uses existing project dependencies (Typer, Rich, pytest) with established patterns.
...
- `tests/test_cli_screener.py` — 4 CLI tests with CliRunner mock patterns
- `tests/test_display.py` — table rendering tests, helper fixtures
- `tests/test_options_chain.py:637-675` — yield column test (most recent column addition)
